...
export class SubscriptionService implements OnApplicationBootstrap {
    ...
    async onApplicationBootstrap() {
        this.logger.log('Server started: Running immediate subscription expiration check...');
    ...
    async update(id: string, updateSubscriptionDto: UpdateSubscriptionDto, user: User): Promise<Subscription> {
    ...
    async handleSubscriptionExpirations() {
        ...
        try {
            ...
            if (expiredSubscriptions.length > 0) {
                ...
                this.logger.log(`Successfully expired ${expiredSubscriptions.length} subscriptions.`);
            } else {
                this.logger.log('No expired subscriptions found today.');
            }
        } catch (error) {
            this.logger.error('Error occurred while attempting to check/expire subscriptions', error);
        }
    }
}
