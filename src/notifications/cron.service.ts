...
export class CronService {
    ...
    async handleWeeklySummary() {
        ...
        const users = await this.userRepository.find({ where: { emailDigest: true } });
    ...
    async handleTrialAlerts() {
        ...
        const users = await this.userRepository.find({ where: { trialAlerts: true } });
    ...
    async handleBudgetAlerts() {
        ...
        for (const user of users) {
            ...
            if (totalMonthlySpend > threshold) {
                this.logger.warn(`[Budget Alert] ${user.email} has exceeded 85% of their budget. Threshold: ${threshold}, Projected: ${totalMonthlySpend}`);
