...
export class UsersService {
    ...
    async update(id: string, updateData: Partial<User>): Promise<User> {
        const { displayName, professionalRole, reportingCurrency, avatarUrl } = updateData;
        const cleanData = { displayName, professionalRole, reportingCurrency, avatarUrl };
    ...
    async getBudgetAnalytics(userId: string) {
        ...
        const subscriptions = await this.usersRepository.manager.find('Subscription', {
        ...
        const lastMonthSpend = lastMonthSubscriptions.reduce((acc, s) => acc + Number(s.cost), 0);
        const currentMonthSpend = currentMonthSubscriptions.reduce((acc, s) => acc + Number(s.cost), 0);
        const avgMonthlySpend = subscriptions.length > 0 ? (subscriptions.reduce((acc, s) => acc + Number(s.cost), 0) / 6) : 0;
        const underBudgetPerc = user.monthlyLimit > 0 ? Math.round(((user.monthlyLimit - lastMonthSpend) / user.monthlyLimit) * 100) : 0;
        const currentBudgetPerc = user.monthlyLimit > 0 ? Math.round((currentMonthSpend / user.monthlyLimit) * 100) : 0;
        ...
        return {
            ...
            underBudgetPerc,
            currentBudgetPerc,
            expiringTrialsCount: subscriptions.filter(s => s.isTrial && new Date(s.trialEndDate) <= new Date(now.getTime() + 7 * 24 * 60 * 60 * 1000)).length,
        };
    }
}
